...
const REPORT_COLUMNS = [
  ...
  { key: 'qty', labelKey: 'r_col_quantity' },
  { key: 'orderNumber', labelKey: 'e_batch_meta_order' },
...
function getRowValue(d, key) {
  ...
  if (key === 'description') return d.productDescription || d.pdfName || '';
  if (key === 'store') return d.store || '';
  if (key === 'qty') return d.qty ?? null;
  if (key === 'orderNumber') return d.orderNumber || '';
  if (key === 'date') return d.date || d.createdAt || '';
  if (key === 'createdBy') return d.createdByEmail || '';
  return '';
}

export default function DeliveryShortagesView({ canDelete = false }) {
  const { t } = useLanguage();
  ...
  async function handleDelete(id) {
    setDeleteError('');
    try {
      await DeliveryShortages.remove(id);
  ...
  const storeOptions = useMemo(() => {
    const set = new Set();
    rows.forEach((d) => d.store && set.add(d.store));
  ...
  const filtered = useMemo(() => {
    let list = rows;
    if (storeFilter !== 'all') list = list.filter((d) => d.store === storeFilter);
    if (search.trim()) {
      const q = search.trim().toLowerCase();
      list = list.filter((d) =>
        (d.productItemCode || '').toLowerCase().includes(q) ||
        (d.productDescription || '').toLowerCase().includes(q) ||
        (d.pdfName || '').toLowerCase().includes(q)
      );
    }
    list = list.filter((d) =>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      {!loading && !error && (
        <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', padding: '14px 20px', background: '#fff', borderBottom: '1px solid #e1e5ea', flexShrink: 0 }}>
          <div>
            <div style={{ fontSize: 24, fontWeight: 700, color: '#c98a1f' }}>{summary.totalQty}</div>
            <div style={{ fontSize: 11.5, color: '#6b7684' }}>{t('ds_total_qty')}</div>
          </div>
          <div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', background: '#f9fafb' }}>
        <p style={{ color: '#6b7684', fontSize: 12.5, margin: '0 0 14px' }}>{t('ds_hint')}</p>
        {deleteError && (
        ...
        {loading ? (
          ...
          <p style={{ color: '#97a2b0' }}>{t('ds_no_results')}</p>
        ) : (
            ...
            <tbody>
              {filtered.map((d) => (
                <tr key={d.id} style={{ borderTop: '1px solid #eef1f4' }}>
                  ...
                  <td style={{ padding: '10px 12px', color: '#3a4353' }}>{d.productDescription || d.pdfName || '—'}</td>
                  <td style={{ padding: '10px 12px' }}>{d.store}</td>
                  <td style={{ padding: '10px 12px' }}>{d.qty ?? '—'}</td>
                  <td style={{ padding: '10px 12px', color: '#6b7684' }}>{d.orderNumber || '—'}</td>
                  <td style={{ padding: '10px 12px', whiteSpace: 'nowrap' }}>{formatDate(d.date || d.createdAt)}</td>
                  <td style={{ padding: '10px 12px', color: '#6b7684' }}>{d.createdByEmail || '—'}</td>
                  {canDelete && (

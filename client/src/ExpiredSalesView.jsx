...
import { ExpiredSales } from '../api.js';
...
function getRowValue(d, key) {
  ...
  if (key === 'createdBy') return d.soldByEmail || '';
  return '';
}

export default function ExpiredSalesView({ canDelete = false }) {
  const { t } = useLanguage();
  ...
  function exportPDF() {
    ...
    doc.setFontSize(12);
    doc.text(`Quick & Fresh — ${t('title_expired_sales')}`, 14, 12);
    autoTable(doc, {
      startY: 18,
      head: [[t('r_col_itemCode'), t('r_col_description'), t('r_col_store'), t('r_col_quantity'), t('x_col_date'), t('r_col_createdBy')]],
      body: filtered.map((d) => [
        d.productItemCode || '',
    ...
    doc.save(`quick-fresh-poulithikan-${new Date().toISOString().slice(0, 10)}.pdf`);
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ padding: '14px 20px', borderBottom: '1px solid #e1e5ea', background: '#fff', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0, flexWrap: 'wrap' }}>
        <strong style={{ fontSize: 15 }}>{t('title_expired_sales')}</strong>
        <button className="btn-primary" style={{ background: '#2f8f8a' }} onClick={exportPDF} title={t('common_export_pdf')}>
          PDF
        </button>
        <input
          value={search}
      ...
      {!loading && !error && (
        <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', padding: '14px 20px', background: '#fff', borderBottom: '1px solid #e1e5ea', flexShrink: 0 }}>
          <div>
            <div style={{ fontSize: 24, fontWeight: 700, color: '#2f8f8a' }}>{summary.totalQty}</div>
            <div style={{ fontSize: 11.5, color: '#6b7684' }}>{t('xs_total_qty')}</div>
          </div>
          <div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', background: '#f9fafb' }}>
        <p style={{ color: '#6b7684', fontSize: 12.5, margin: '0 0 14px' }}>{t('xs_hint')}</p>
        {deleteError && (
        ...
        {loading ? (
          ...
          <p style={{ color: '#97a2b0' }}>{t('xs_no_results')}</p>
        ) : (
            ...
            <tbody>
              {filtered.map((d) => (
                <tr key={d.id} style={{ borderTop: '1px solid #eef1f4' }}>
                  ...
                  <td style={{ padding: '10px 12px', color: '#6b7684' }}>{d.soldByEmail || '—'}</td>
                  {canDelete && (
